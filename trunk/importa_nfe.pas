...
uses
  ...
  DBAccess, pcnConversao, pcnNFeW, Menus, RzPrgres, ImgList, wwdblook,
...
uses ACBrNFeNotasFiscais, pcnNFe, MaskUtils, loc_conta, modulo, principal,
  Produto, produto_ligth, produto_farma, produto_auto, UFuncoes, uBuscaXml, NCM,
  cst, MargemLucro;
...
begin
  ...
  begin
    ...
    begin
        ...
        recupera := true;
      {
        ...
        ShowMessage('Chave de Acesso não pode ser vazia');
        eid.SetFocus; }
    ...
    begin
      ...
      if not(Tag = 3) then
      begin
        frmBuscarXML := TfrmBuscarXML.create(self);
        frmBuscarXML.ShowModal;

        // OpenDialog1.FileName := ACBrNFe1.Configuracoes.Geral.PathSalvar+'\'+eid.Text;
        OpenDialog1.FileName := frmBuscarXML.FPath;
      end
      else
        recupera := true;
    end;
  ...
  // if OpenDialog1.Execute  or recupera then
  if recupera then
  ...
  begin
    ...
    inicia: { : INICIA VERIFICAÇÃO SE O PRODUTO ESTÁ PRONTO PARA RECEBER MOVIMENTAÇÃO - HISTORICO DE COMPRAS }
      ...
      begin
        ...
        qrproduto_movCONTROLE.AsString := qritem.fieldbyname('codnota')
          .AsString;
...
begin
  ...
  try
    ...
    try
      ...
      qritemPRECOCUSTO.ASFLOAT := (qritemTOTAL.ASFLOAT + qritemFRETE.ASFLOAT +
        qritemSEGURO.ASFLOAT + qritemOUTRAS.ASFLOAT + qritemIPI_VALOR.ASFLOAT) /
        (qritemQTDE.ASFLOAT) / qritemFRACAO.ASFLOAT;
...
begin
  ...
  fximprime.LoadFromFile('\Z5\server\REL\F000197.fr3');
