...
uses modulo, principal, loc_funci, loc_caixa;
...
procedure Tfrmusuario.AdvGlowButton1Click(Sender: TObject);
begin

  qrapoio.close;
  qrapoio.sql.clear;
  qrapoio.sql.add('select * from c000067 where codusuario = ''' + DBEdit1.Text + '''');
  qrapoio.Open;

  //funcao para passar tudo pra sim
  ...
  begin
    ...
    begin
      qrapoio.edit;
      qrapoio.fieldbyname('acesso').asstring := 'SIM';
      qrapoio.POST;
      qrapoio.Next;
    end;
  end;
...
procedure Tfrmusuario.S1Click(Sender: TObject);
begin
...
procedure Tfrmusuario.RzDBButtonEdit1Exit(Sender: TObject);
begin
  tedit(sender).Color := clwindow;
  if (frmmodulo.qrusuario.state = dsinsert) or (frmmodulo.qrusuario.State = dsedit) then
  begin
    ...
    if dbedit29.text <> '000000' then
    begin
      if not FrmPrincipal.Locregistro(frmmodulo.qrcaixa_operador, dbedit29.text, 'CODFUNCIONARIO') then
        RzDBButtonEdit1ButtonClick(Frmusuario)
      else
      begin
        perform(wm_nextdlgctl, 0, 0);
      end;
...
procedure Tfrmusuario.RzDBButtonEdit1ButtonClick(Sender: TObject);
begin
  ...
  frmmodulo.qrusuario.fieldbyname('COD_CAIXA').asstring := resultado_pesquisa1;
end;

procedure Tfrmusuario.RzDBButtonEdit1KeyPress(Sender: TObject;
  var Key: Char);
begin
  if key = #13 then perform(wm_nextdlgctl, 0, 0);
end;
