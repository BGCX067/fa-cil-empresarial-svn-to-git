 ...
 //   QRORCAMENTO_PRODUTO.OPEN;
  ...
  begin
    ...
    begin
      ...
      begin
        APPLICATION.MESSAGEBOX(PWideChar('O produto ' + qrorcamento_produto.fieldbyname('codproduto').asstring + ' - ' +
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        texto := PWideChar('Cliente com crédito em Observação!, Deseja continuar?');
        if application.MessageBox(texto, 'Atenção', mb_yesno + MB_ICONERROR) = idyes then
        begin
          if not frmPrincipal.autentica('Liberar venda', 4) then
          begin
      ...
      begin
        ...
        begin
          texto := PWideChar('Cliente com nome no SPC!, Deseja continuar?');
          if application.MessageBox(texto, 'Atenção', mb_yesno + MB_ICONERROR) = idyes then
          begin
            if not frmPrincipal.autentica('Liberar venda', 4) then
            begin
