...
uses
  ...
  Menus, AdvGlowButton, TFlatPanelUnit, RxToolEdit;
...
begin
  ...
  begin
    ...
    memo1.text := '';
    ZipMaster1.ZipFileName := edestino.text + 'iCloud_' + copy(datetostr(date), 7, 4) + copy(datetostr(date), 4, 2) + copy(datetostr(date), 1, 2) + '_' + copy(timetostr(time), 1, 2) + copy(timetostr(time), 4, 2) + '.zip'; ;
