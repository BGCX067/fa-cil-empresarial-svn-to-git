object frmProd_perigoso_cte: TfrmProd_perigoso_cte
  Left = 459
  Top = 254
  BorderIcons = []
  BorderStyle = bsSingle
  Caption = 'Produto Perigoso'
  ClientHeight = 274
  ClientWidth = 525
  Color = clBtnFace
  Font.Charset = DEFAULT_CHARSET
  Font.Color = clWindowText
  Font.Height = -11
  Font.Name = 'MS Sans Serif'
  Font.Style = []
  OldCreateOrder = False
  PopupMenu = pop1
  Position = poMainFormCenter
  PixelsPerInch = 96
  TextHeight = 13
  object RzPanel2: TRzPanel
    Left = 0
    Top = 213
    Width = 525
    Height = 61
    Align = alBottom
    BorderOuter = fsNone
    BorderColor = clBlack
    BorderWidth = 1
    Color = clWhite
    TabOrder = 0
    object btincluir: TAdvGlowButton
      Left = 160
      Top = 3
      Width = 93
      Height = 55
      Caption = 'F2 | Salvar'
      Font.Charset = ANSI_CHARSET
      Font.Color = clWindowText
      Font.Height = -11
      Font.Name = 'Tahoma'
      Font.Style = []
      ImageIndex = 49
      FocusType = ftHot
      NotesFont.Charset = DEFAULT_CHARSET
      NotesFont.Color = clWindowText
      NotesFont.Height = -11
      NotesFont.Name = 'Tahoma'
      NotesFont.Style = []
      ParentFont = False
      Picture.Data = {
        
        82}
      Transparent = True
      TabOrder = 1
      OnClick = btincluirClick
      Appearance.BorderColor = 14727579
      Appearance.BorderColorHot = 10079963
      Appearance.BorderColorDown = 4548219
      Appearance.BorderColorChecked = clBlack
      Appearance.Color = 15653832
      Appearance.ColorTo = 16178633
      Appearance.ColorChecked = 7915518
      Appearance.ColorCheckedTo = 11918331
      Appearance.ColorDisabled = 15921906
      Appearance.ColorDisabledTo = 15921906
      Appearance.ColorDown = 7778289
      Appearance.ColorDownTo = 4296947
      Appearance.ColorHot = 15465983
      Appearance.ColorHotTo = 11332863
      Appearance.ColorMirror = 15586496
      Appearance.ColorMirrorTo = 16245200
      Appearance.ColorMirrorHot = 5888767
      Appearance.ColorMirrorHotTo = 10807807
      Appearance.ColorMirrorDown = 946929
      Appearance.ColorMirrorDownTo = 5021693
      Appearance.ColorMirrorChecked = 10480637
      Appearance.ColorMirrorCheckedTo = 5682430
      Appearance.ColorMirrorDisabled = 11974326
      Appearance.ColorMirrorDisabledTo = 15921906
      Appearance.GradientHot = ggVertical
      Appearance.GradientMirrorHot = ggVertical
      Appearance.GradientDown = ggVertical
      Appearance.GradientMirrorDown = ggVertical
      Appearance.GradientChecked = ggVertical
    end
    object AdvGlowButton14: TAdvGlowButton
      Left = 253
      Top = 3
      Width = 107
      Height = 55
      Caption = 'ESC | Sair'
      Font.Charset = ANSI_CHARSET
      Font.Color = clWindowText
      Font.Height = -11
      Font.Name = 'Tahoma'
      Font.Style = []
      ImageIndex = 49
      FocusType = ftHot
      NotesFont.Charset = DEFAULT_CHARSET
      NotesFont.Color = clWindowText
      NotesFont.Height = -11
      NotesFont.Name = 'Tahoma'
      NotesFont.Style = []
      ParentFont = False
      Picture.Data = {
        
        A13EB9F389EDB60000000049454E44AE426082}
      Transparent = True
      TabOrder = 0
      OnClick = AdvGlowButton14Click
      Appearance.BorderColor = 14727579
      Appearance.BorderColorHot = 10079963
      Appearance.BorderColorDown = 4548219
      Appearance.BorderColorChecked = clBlack
      Appearance.Color = 15653832
      Appearance.ColorTo = 16178633
      Appearance.ColorChecked = 7915518
      Appearance.ColorCheckedTo = 11918331
      Appearance.ColorDisabled = 15921906
      Appearance.ColorDisabledTo = 15921906
      Appearance.ColorDown = 7778289
      Appearance.ColorDownTo = 4296947
      Appearance.ColorHot = 15465983
      Appearance.ColorHotTo = 11332863
      Appearance.ColorMirror = 15586496
      Appearance.ColorMirrorTo = 16245200
      Appearance.ColorMirrorHot = 5888767
      Appearance.ColorMirrorHotTo = 10807807
      Appearance.ColorMirrorDown = 946929
      Appearance.ColorMirrorDownTo = 5021693
      Appearance.ColorMirrorChecked = 10480637
      Appearance.ColorMirrorCheckedTo = 5682430
      Appearance.ColorMirrorDisabled = 11974326
      Appearance.ColorMirrorDisabledTo = 15921906
      Appearance.GradientHot = ggVertical
      Appearance.GradientMirrorHot = ggVertical
      Appearance.GradientDown = ggVertical
      Appearance.GradientMirrorDown = ggVertical
      Appearance.GradientChecked = ggVertical
    end
  end
  object sGroupBox1: TsGroupBox
    Left = 0
    Top = 0
    Width = 525
    Height = 213
    Align = alClient
    TabOrder = 1
    SkinData.SkinSection = 'GROUPBOX'
    object Label1: TLabel
      Left = 8
      Top = 24
      Width = 76
      Height = 13
      Caption = 'Classe de Risco'
      FocusControl = classerisco
    end
    object Label2: TLabel
      Left = 136
      Top = 24
      Width = 64
      Height = 13
      Caption = 'N'#250'mero ONU'
      FocusControl = onu
    end
    object Label3: TLabel
      Left = 264
      Top = 24
      Width = 96
      Height = 13
      Caption = 'Grupo e Embalagem'
      FocusControl = gremb
    end
    object Label4: TLabel
      Left = 392
      Top = 24
      Width = 75
      Height = 13
      Caption = 'Ponto de Fulgor'
      FocusControl = pontofulgor
    end
    object Label5: TLabel
      Left = 8
      Top = 160
      Width = 140
      Height = 13
      Caption = 'Quantidade Total por Produto'
      FocusControl = qtdetot
    end
    object Label6: TLabel
      Left = 184
      Top = 160
      Width = 147
      Height = 13
      Caption = 'Quantidade e tipos de Volumes'
      FocusControl = tipos
    end
    object Label7: TLabel
      Left = 10
      Top = 72
      Width = 211
      Height = 13
      Caption = 'Nome Apropriado para embarque do Produto'
      FocusControl = classerisco
    end
    object classerisco: TcxDBTextEdit
      Left = 8
      Top = 40
      DataBinding.DataField = 'CLASE_RISCO'
      DataBinding.DataSource = DataSource1
      TabOrder = 0
      Width = 121
    end
    object onu: TcxDBTextEdit
      Left = 136
      Top = 40
      DataBinding.DataField = 'NUMERO_ONU'
      DataBinding.DataSource = DataSource1
      TabOrder = 1
      Width = 121
    end
    object gremb: TcxDBTextEdit
      Left = 264
      Top = 40
      DataBinding.DataField = 'GRUPO_EMBALAGEM'
      DataBinding.DataSource = DataSource1
      TabOrder = 2
      Width = 121
    end
    object pontofulgor: TcxDBTextEdit
      Left = 392
      Top = 40
      DataBinding.DataField = 'PONTO_FULGOR'
      DataBinding.DataSource = DataSource1
      TabOrder = 3
      Width = 121
    end
    object qtdetot: TcxDBCalcEdit
      Left = 8
      Top = 176
      DataBinding.DataField = 'QUANT_TOT_PROD'
      DataBinding.DataSource = DataSource1
      TabOrder = 5
      Width = 153
    end
    object tipos: TcxDBCalcEdit
      Left = 184
      Top = 176
      DataBinding.DataField = 'QUANT_TIP_VOL'
      DataBinding.DataSource = DataSource1
      TabOrder = 6
      Width = 153
    end
    object dbmemo: TDBMemo
      Left = 8
      Top = 89
      Width = 505
      Height = 65
      DataField = 'NOME_APROPRIADO'
      DataSource = DataSource1
      TabOrder = 4
    end
  end
  object DataSource1: TDataSource
    Left = 392
    Top = 240
  end
  object ACBrEnterTab1: TACBrEnterTab
    EnterAsTab = True
    Left = 432
    Top = 237
  end
  object pop1: TPopupMenu
    Left = 472
    Top = 237
    object S1: TMenuItem
      Caption = 'Salvar'
      ShortCut = 113
      OnClick = btincluirClick
    end
    object S2: TMenuItem
      Caption = 'Sair'
      ShortCut = 27
      OnClick = AdvGlowButton14Click
    end
  end
end
