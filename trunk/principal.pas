...
uses
  ...
  MemDS, StrUtils, Mask, RzEdit, RzPanel, IBServices, DBAccess,
  ...
  sSpeedButton,
  dxSkinsForm, ACBrBase, ACBrValidador,
  UCDataConnector, UCZEOSConn, UCBase, UCSettings, UCMail,
  TLHelp32, acApiUtils, ResizeKit, Vcl.ActnList, Vcl.ActnMan, Vcl.ActnCtrls,
  Vcl.Ribbon,
  Vcl.RibbonLunaStyleActnCtrls, Vcl.ActnMenus, Vcl.RibbonActnMenus,
  Vcl.StdActns, Vcl.RibbonObsidianStyleActnCtrls,
  Vcl.RibbonSilverStyleActnCtrls, dxSkinBlack,
  ...
  dxSkinXmas2008Blue,
  AdvMetroTile, System.IniFiles, dxRibbonSkins, dxSkinOffice2013White,
  ...
  dxStatusBar,
  dxRibbonStatusBar, dxCustomTileControl, dxTileControl, LockApplication,
  faleconosco;
...
type
  ...
  TfrmPrincipal = class(TForm)
    ...
    vversao: TsPanel;
    Image4: TImage;
    IconeAtalhoCaixa: TdxTileControlItem;
    IconeAtalhoReceber: TdxTileControlItem;
    IconeAtalhoPagar: TdxTileControlItem;
    IconeAtalhoProdutos: TdxTileControlItem;
    IconeAtalhoClientes: TdxTileControlItem;
    IconeAtalhoOrcamento: TdxTileControlItem;
    IconeAtalhoOS: TdxTileControlItem;
    dxTileItem1: TdxTileControlItem;
    dxTileItem2: TdxTileControlItem;
    IconeAtalhoVendas: TdxTileControlItem;
    AtalhoIconeCaixa: TdxTileControlItem;
    dxTileGroup1: TdxTileControlGroup;
    AtalhoIconePagar: TdxTileControlItem;
    AtalhoIconeReceber: TdxTileControlItem;
    AtalhoIconeClientes: TdxTileControlItem;
    AtalhoIconeOS: TdxTileControlItem;
    AtalhoIconeVendas: TdxTileControlItem;
    AtalhoIconeOrcamento: TdxTileControlItem;
    AtalhoIconeProdutos: TdxTileControlItem;
    ...
    procedure IconeAtalhoCaixaClick(Sender: TdxTileControlItem);
    ...
    procedure IconeAtalhoPagarClick(Sender: TdxTileControlItem);
    procedure IconeAtalhoReceberClick(Sender: TdxTileControlItem);
    procedure IconeAtalhoProdutosClick(Sender: TdxTileControlItem);
    procedure IconeAtalhoClientesClick(Sender: TdxTileControlItem);
    procedure IconeAtalhoOrcamentoClick(Sender: TdxTileControlItem);
    procedure IconeAtalhoOSClick(Sender: TdxTileControlItem);
    procedure IconeAtalhoVendasClick(Sender: TdxTileControlItem);
    procedure AtalhoIconeCaixaClick(Sender: TdxTileControlItem);
    procedure AtalhoIconePagarClick(Sender: TdxTileControlItem);
    procedure AtalhoIconeReceberClick(Sender: TdxTileControlItem);
    procedure AtalhoIconeClientesClick(Sender: TdxTileControlItem);
    procedure AtalhoIconeOSClick(Sender: TdxTileControlItem);
    procedure AtalhoIconeVendasClick(Sender: TdxTileControlItem);
    procedure AtalhoIconeProdutosClick(Sender: TdxTileControlItem);
    procedure AtalhoIconeOrcamentoClick(Sender: TdxTileControlItem);
  ...
  public
    ...
    // Procedure Mensagem(Msg, Titulo: String);
    procedure Aguarde(sTitulo, sTexto: string;
      FechaAutomatico, bMostra: Boolean; TimeOut: LongInt);
...
procedure AcertaPadraoData; External '\Z5\server\dll\ebo7.dll';

function NumeroSerieHD: string; External '\Z5\server\dll\ebo7.dll';
function frm(ebo: string): string; External '\Z5\server\dll\ebo7.dll';
...
uses modulo, senha, filial, papelparede, cliente,
  ...
  Bombas, Encerrantes,
  Combustivel, LMCImpressao, UAguarde, UfrmManifestacao, novidades_versao,
  ajuda, ROTINAS_GERAL; // , UProtect
...
begin
  ...
  begin
    continua := false;
    application.messagebox
      ('Erro de comunicaÁ„o, a FunÁ„o nao conseguiu enviar o comando', 'Erro',
      mb_ok + MB_ICONERROR);
  end;
  ...
  begin
    continua := false;
    application.messagebox('Erro de ExecuÁ„o da FunÁ„o', 'Erro',
      mb_ok + MB_ICONERROR);
  end;
  ...
  begin
    continua := false;
    application.messagebox('Par‚metro inv·lido passado na funÁ„o.!', 'Erro',
      mb_ok + MB_ICONERROR);
  end;
  ...
  begin
    continua := false;
    application.messagebox('AlÌquota n„o programada no ECF. !', 'Erro',
      mb_ok + MB_ICONERROR);
  end;
  ...
  begin
    continua := false;
    application.messagebox('Erro ao Abrir a Porta de ComunicaÁ„o', 'Erro',
      mb_ok + MB_ICONERROR);
  end;
  ...
  begin
    continua := false;
    application.messagebox
      ('Impressora desligada ou cabo de comunicaÁ„o desconectado.', 'Erro',
      mb_ok + MB_ICONERROR);
  end;
...
begin
  ...
  begin
    ...
    if Length(Daruma_MSG) > 1 then
      application.messagebox(PWideChar(Daruma_MSG), 'AtenÁ„o',
  ...
  begin
    application.messagebox
      ('Erro Fatal na Impressora! A aplicaÁ„o ser· finalizada!', 'Erro',
      mb_ok + MB_ICONERROR);
...
begin
  ...
  begin
    ...
    begin
      iST1 := iST1 - 64;
      application.messagebox('Papel est· terminando', 'AtenÁ„o',
        mb_ok + mb_iconwarning);
      iRet := 1;
      continua := true;
    end
    ...
    begin
      ...
      begin
        iST1 := iST1 - 32;
        application.messagebox('Erro no relÛgio interno do ECF', 'Erro',
          mb_ok + MB_ICONERROR);
      end;
      if iST1 = 16 then
      begin
      ...
      begin
        iST2 := iST2 - 64;
        application.messagebox('MemÛria Fiscal cheia', 'Erro',
          mb_ok + MB_ICONERROR);
        continua := false;
      end;
      if iST2 = 32 then
      ...
      begin
        iST2 := iST2 - 16;
        application.messagebox('AlÌquota n„o programada', 'Erro',
          mb_ok + MB_ICONERROR);
        continua := false;
      end;
      if iST2 = 8 then
      begin
        iST2 := iST2 - 8;
        application.messagebox('Capacidade de alÌquotas esgotada', 'Erro',
          mb_ok + MB_ICONERROR);
        continua := false;
      end;
      if iST2 = 5 then
  ...
  if iACK = 21 then
  begin
    application.messagebox(PWideChar('AtenÁ„o!!!' + #13 + #10 +
...
begin
  ...
  if retorno = 0 then
    application.messagebox('Erro de ComunicaÁ„o !', 'Erro',
      mb_ok + MB_ICONERROR);
  if retorno = -1 then
    application.messagebox('Erro de ExecuÁ„o na FunÁ„o. Verifique!', 'Erro',
      mb_ok + MB_ICONERROR);
  ...
  if retorno = -18 then
    application.messagebox('N„o foi possÌvel abrir arquivo INTPOS.001 !',
      'Erro', mb_ok + MB_ICONERROR);
  ...
  if retorno = -20 then
    application.messagebox('TransaÁ„o cancelada pelo Operador !', 'Erro',
      mb_ok + MB_ICONERROR);

  if retorno = -21 then
    application.messagebox('A TransaÁ„o n„o foi aprovada !', 'Erro',
      mb_ok + MB_ICONERROR);

  if retorno = -22 then
    application.messagebox('N„o foi possÌvel terminal a Impress„o !', 'Erro',
      mb_ok + MB_ICONERROR);

  if retorno = -23 then
    application.messagebox('N„o foi possÌvel terminal a OperaÁ„o !', 'Erro',
      mb_ok + MB_ICONERROR);
  ...
  if retorno = -26 then
    application.messagebox('TransaÁ„o j· Efetuada !', 'Erro',
      mb_ok + MB_ICONERROR);

  if retorno = -28 then
    application.messagebox('N„o h· InformaÁıes para serem Impressas !', 'Erro',
      mb_ok + MB_ICONERROR);
  result := retorno;
end;
...
begin
  ...
  try
    if FileExists(File_Path) then
      ...
      (Pchar('N„o foi possÌvel encontrar o arquivo - ' + #13 + File_Path +
      #13#13 + 'Deseja Localizar o Arquivo?'), 'AtenÁ„o',
  ...
  case retorno of
    0:
      Error_Msg := 'Erro: N„o h· memÛria sufiente para executar o arquivo - ' +
        File_Path;
    ...
    SE_ERR_ASSOCINCOMPLETE:
      Error_Msg :=
        'Erro: AssociaÁ„o de tipo de arquivo incompatÌvel ou inv·lida.';
    SE_ERR_DDEBUSY:
      Error_Msg :=
        'Erro: TransaÁ„o DDE n„o pode ser completada devido a execuÁ„o de outras transaÁıes.';
    SE_ERR_DDEFAIL:
...
begin
  ...
  begin
    showmessage('N„o foi possÌvel concluir com a operaÁ„o!' + #13 +
      'Erro: CÛdigo de sequÍncia n„o encontrado na tabela de cÛdigos.');
...
begin
  ...
  try
    ...
    if not Registro.OpenKey('F·cil Empresarial', false) then
    begin
      Registro.CreateKey('F·cil Empresarial');
      Registro.OpenKey('F·cil Empresarial', true);
  ...
  begin
    ...
    data := DataDeCriacao('C:\Z5\server\Athenas.exe');
    ...
    begin
      ...
      begin
        ...
        begin
          // if d > 2 then
          // begin
          // label8.caption := 'H· ' + inttostr(d) + ' dias n„o È feito';
          // alerta_copia.Popup;
          // end;
        end;
...
begin
  ...
  try
    ...
    if not DirectoryExists('c:\Z5\server\atualizacao') then
      if application.messagebox
        ('Caminho do arquivo para AtualizaÁ„o n„o encontrado, Deseja Cria-lo?',
        'Aviso', MB_YESNO + mb_iconwarning) = idYes then
      begin
        ForceDirectories('c:\Z5\server\atualizacao');
        application.messagebox('Caminho Criado com Sucesso!', 'Aviso!',
          mb_ok + mb_iconquestion);
      end;

    assignfile(txt, '\Z5\server\ini\com.ini');
    reset(txt);
    ...
    ExeAtualiza := ExtractFilePath(ParamStr(0)) + 'atualizacao\Athenas.exe';
    ExeAtual := ExtractFilePath(ParamStr(0)) + 'Athenas.exe';
    ...
    begin
      ...
      begin
        if application.messagebox
          (Pchar('Foi Encontrada uma nova vers„o do Sistema Athenas.' + #13 +
        ...
        begin
          ...
          // Informo que a aplicaÁ„o foi atualizado com sucesso
          // copia da pasta atualizacao para a pasta Athenas
          CopyFile(Pchar(ExeAtualiza),
            Pchar('c:\Z5\server\Athenas.exe'), false);
          showmessage('AplicaÁ„o atualizada com sucesso!');
          // Mando abrir a nova vers„o do teclado
          Hand := FindWindow('TApplication', 'F·cil Empresarial');
          if Hand = 0 then
            frmPrincipal.ExecutePrograma
              ('\Z5\server\Facil_Empresarial.exe', '');
          // Fecho a aberta
          // Close;
          application.Terminate;
        end
  ...
  begin
    Try
      Try
        ...
        Action := cafree;
      Except
        killtask('Athenas_retaguarda.exe')
      End;
    Finally
...
begin
  ...
  begin
    application.messagebox('Acesso n„o permitido!', 'AtenÁ„o',
      mb_ok + MB_ICONERROR);
    Exit;
  end;
...
begin
  ...
  frmprogresso.ltexto.caption :=
    'Gerando REGISTRO 60 ANALÕTICO para o SINTEGRA...';
  ...
  begin
    ...
    begin
      ...
      begin
        if copy(Texto, CAMPO_INI, CAMPO_QTDE) = 'N˙mero de sÈrie...........:'
        then
        begin
          Texto := (TrimLeft(copy(Texto, RESULTADO_INI, RESULTADO_QTDE)));
  ...
  begin
    ...
    begin
      ...
      if (copy(Texto, CAMPO_INI, CAMPO_QTDE) = 'Reinicio de OperaÁ„o......:') or
...
begin
  WinExec('C:\Z5\ferramentas\suporte.exe', SW_SHOW);
end;
...
begin
  WinExec('C:\Z5\Server\arquivos\AA_v3.exe', SW_SHOW);
end;

procedure TfrmPrincipal.dxBarButton71Click(Sender: TObject);
begin
...
procedure TfrmPrincipal.dxBarButton72Click(Sender: TObject);
begin
  // frmSplash2.Info := True;
  // frmSplash2.ShowModal;
end;

procedure TfrmPrincipal.dxBarButton73Click(Sender: TObject);
begin

  ShellExecute(Handle, 'open', 'http://www.facebook.com.br/z5sistemas', nil,
    nil, SW_SHOW);
end;

procedure TfrmPrincipal.dxBarButton74Click(Sender: TObject);
begin
  ShellExecute(Handle, 'open',
    'https://www.youtube.com/channel/UCpidOoTjY42WdKdNEMSf1-A', nil,
    nil, SW_SHOW);
end;

procedure TfrmPrincipal.dxBarButton75Click(Sender: TObject);
begin

  ShellExecute(Handle, 'open',
    'https://plus.google.com/116071802691728583361/about', nil, nil, SW_SHOW);
end;

procedure TfrmPrincipal.dxBarButton76Click(Sender: TObject);
begin
  form_sobre := tform_sobre.create(self);
  form_sobre.showmodal;
end;

procedure TfrmPrincipal.dxBarButton77Click(Sender: TObject);
begin
...
begin
  WinExec('C:\Z5\server\Athenas_SPED.exe', SW_SHOW);
  // frmspedfiscal := tfrmspedfiscal.create(self);
  // frmspedfiscal.showmodal;

end;

procedure TfrmPrincipal.dxBarLargeButton31Click(Sender: TObject);
begin
  if USA_TEF then
    frmmodulo.QRCONFIG.open;
...
procedure TfrmPrincipal.dxBarLargeButton32Click(Sender: TObject);
begin
  frmfale_conosco := Tfrmfale_conosco.create(self);
  frmfale_conosco.showmodal;
end;

procedure TfrmPrincipal.dxBarLargeButton33Click(Sender: TObject);
begin
  WinExec('C:\Z5\Remoto\ShowMyPC3160.exe', SW_ShowNormal);
end;

procedure TfrmPrincipal.dxBarLargeButton34Click(Sender: TObject);
begin
  ShellExecute(Handle, 'open', 'http://www.zcinco.com.br', nil, nil, SW_SHOW);
end;

procedure TfrmPrincipal.dxBarLargeButton35Click(Sender: TObject);
begin
  frmPrincipal.LkSeguranca.renovar;
  Label27.caption := '';
  Label27.caption := 'IdentificaÁ„o : ' +
    IntToStr(frmPrincipal.LkSeguranca.IDInstalacao) + #13;
  Label27.caption := Label27.caption + 'Dias Restantes : ' +
    IntToStr(frmPrincipal.LkSeguranca.Dias_RestantesU) + #13;
  Label27.caption := Label27.caption + 'Data Vencimento : ' +
    frmPrincipal.LkSeguranca.Data_VencimentoU + #13;
  Label27.caption := Label27.caption + 'Chave : ' +
    frmPrincipal.LkSeguranca.Chave_RegistradaU + #13;
  Label27.caption := Label27.caption + 'E-mail do cliente : ' +
    frmPrincipal.LkSeguranca.Email_ClienteU + #13;
  // Label27.Caption := Label1.Caption + 'Vers„o : ' + IntToStr(frmPrincipal.ativador.Versao_Sistema )+ #13;
  if frmPrincipal.LkSeguranca.Sistema_DemoU then
    Label27.caption := Label27.caption + 'DEMO'
  else
    Label27.caption := Label27.caption + 'REGISTRADO'
end;

procedure TfrmPrincipal.dxBarLargeButton36Click(Sender: TObject);
begin
  frmEMPRESA := tfrmEMPRESA.create(self);
  frmEMPRESA.showmodal;
end;

procedure TfrmPrincipal.dxBarLargeButton37Click(Sender: TObject);
begin

  frmnovidedasversao := tfrmnovidedasversao.create(self);
  frmnovidedasversao.showmodal;
...
procedure TfrmPrincipal.IconeAtalhoCaixaClick(Sender: TdxTileControlItem);
begin
  if frmPrincipal.autentica_caixa('Acesso ao Caixa', 0) then
  begin
    frmcaixa := tfrmcaixa.create(self);
...
procedure TfrmPrincipal.IconeAtalhoClientesClick(Sender: TdxTileControlItem);
begin
  frmcliente := tfrmcliente.create(self);
  frmcliente.showmodal;
end;

procedure TfrmPrincipal.IconeAtalhoOrcamentoClick(Sender: TdxTileControlItem);
begin
  frmorcamento := tfrmorcamento.create(self);
  frmorcamento.showmodal;
end;

procedure TfrmPrincipal.IconeAtalhoOSClick(Sender: TdxTileControlItem);
begin
  frmos_menu := tfrmos_menu.create(self);
  frmos_menu.showmodal;
end;

procedure TfrmPrincipal.IconeAtalhoPagarClick(Sender: TdxTileControlItem);
begin
    FRMCONTASPAGAR := TFRMCONTASPAGAR.create(self);
  FRMCONTASPAGAR.showmodal;
end;

procedure TfrmPrincipal.IconeAtalhoProdutosClick(Sender: TdxTileControlItem);
begin
  frmmodulo.QRCONFIG.open;
  if frmmodulo.QRCONFIG.FIELDBYNAME('CADASTRO_PRODUTO').asstring = 'NORMAL' then
  begin
...
procedure TfrmPrincipal.IconeAtalhoReceberClick(Sender: TdxTileControlItem);
begin
  frmcontasreceber := tfrmcontasreceber.create(self);
  frmcontasreceber.showmodal;
end;

procedure TfrmPrincipal.IconeAtalhoVendasClick(Sender: TdxTileControlItem);
begin
  if USA_TEF then
    frmmodulo.QRCONFIG.open;
...
procedure TfrmPrincipal.AtalhoIconeCaixaClick(Sender: TdxTileControlItem);
begin
begin
  if frmPrincipal.autentica_caixa('Acesso ao Caixa', 0) then
  begin
...
procedure TfrmPrincipal.AtalhoIconeClientesClick(Sender: TdxTileControlItem);
begin
  frmcliente := tfrmcliente.create(self);
  frmcliente.showmodal;
end;

procedure TfrmPrincipal.AtalhoIconeOrcamentoClick(Sender: TdxTileControlItem);
begin
  frmorcamento := tfrmorcamento.create(self);
  frmorcamento.showmodal;
end;

procedure TfrmPrincipal.AtalhoIconeOSClick(Sender: TdxTileControlItem);
begin
  frmos_menu := tfrmos_menu.create(self);
  frmos_menu.showmodal;
end;

procedure TfrmPrincipal.AtalhoIconePagarClick(Sender: TdxTileControlItem);
begin
  FRMCONTASPAGAR := TFRMCONTASPAGAR.create(self);
  FRMCONTASPAGAR.showmodal;
end;

procedure TfrmPrincipal.AtalhoIconeProdutosClick(Sender: TdxTileControlItem);
begin
  frmmodulo.QRCONFIG.open;
  if frmmodulo.QRCONFIG.FIELDBYNAME('CADASTRO_PRODUTO').asstring = 'NORMAL' then
  begin
...
procedure TfrmPrincipal.AtalhoIconeReceberClick(Sender: TdxTileControlItem);
begin
  frmcontasreceber := tfrmcontasreceber.create(self);
  frmcontasreceber.showmodal;
end;

procedure TfrmPrincipal.AtalhoIconeVendasClick(Sender: TdxTileControlItem);
begin
  if USA_TEF then
    frmmodulo.QRCONFIG.open;
...
begin
  ...
  frmPrincipal.ExecutePrograma('\Z5\server\sped.exe', '');
end;
...
begin
  ...
  begin
    application.messagebox('MÛdulo N„o Liberado!', 'AtenÁ„o',
      mb_ok + MB_ICONERROR);
    Exit;
  end;
...
begin
  ...
  begin
    application.messagebox('Seu nÌvel n„o permite este acesso!', 'AtenÁ„o',
      mb_ok + mb_iconwarning);
    Exit;
  end
  else
...
begin
  application.messagebox(Pchar('Prezado Cliente.O Atualizador ser· inciado,' +
    #13 + 'ApÛs a sua execuÁ„o, se for feito o download de uma nova vers„o,' +
    #13 + 'vocÍ deve reiniciar o sistema, para que a atualizaÁ„o tenha efeito!!'),
    'AtenÁ„o - Leia [**Muito importante**]', mb_ok + MB_ICONEXCLAMATION);

  Hand := FindWindow('TApplication', 'Atualizador');
  if Hand = 0 then
    frmPrincipal.ExecutePrograma
      ('\Z5\server\Atualizacao\Athenas_Atualizador.exe', '');
...
begin
  { showmessage('VocÍ ficou mais de 5 minutos inativo' + #13 +
    'Por seguranÁa faÁa Login novamente'); }
end;
...
procedure TfrmPrincipal.FechaAguarde;
begin
  Aguarde('', '', false, false, 0);
end;
...
procedure TfrmPrincipal.Aguarde(sTitulo, sTexto: string;
  FechaAutomatico, bMostra: Boolean; TimeOut: LongInt);
begin
  ...
  FAguarde.lbTitulo.caption := sTitulo;
  FAguarde.lbTexto.caption := sTexto;
  ...
  if FechaAutomatico then
  begin
    FAguarde.show;
    FAguarde.iTempo := TimeOut;
    FAguarde.Timer1.Enabled := true;
  end;
