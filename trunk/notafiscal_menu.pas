...
uses
  ...
  inifiles, AdvReflectionImage, RxToolEdit, UCBase, ACBrNFeDANFEFR;
...
uses modulo, principal, compra, cfop, cst, modelo_fiscal, xloc_cliente,
  ...
  status, nfe_legenda, email, math, cliente, produto, produto_ligth,
  produto_farma, produto_auto, fornecedor, empresa, OpNatureza, orcamento,
  notas_venda;
...
begin
  ...
  begin
    ...
    //ACBrNFeDANFERave1.Site:= query2.fieldbyname('EMAIL').asstring;
    ...
    begin
      ...
      begin
        ...
        begin
          ...
          if qryapoio.fieldbyname('natureza').asstring = 'venda' then
          begin
            if frmmodulo.qrproduto.Locate('codigo',
              qrnota_item.fieldbyname('codproduto').asstring,
              [loCaseInsensitive]) then
            begin
              if qrNFE_Cliente.fieldbyname('uf').asstring <>
                query2.fieldbyname('UF').asstring then
                Prod.cfop := frmmodulo.qrproduto.fieldbyname
                  ('IND_CFOP_VENDA_FORA').asstring
              else
                Prod.cfop := frmmodulo.qrproduto.fieldbyname
                  ('IND_CFOP_VENDA_DENTRO').asstring;
            end;
            Ide.natOp := frmmodulo.qrconfig.fieldbyname
              ('TEXTO_PADRAO_VENDA').asstring;
          end;

          if qryapoio.fieldbyname('natureza').asstring = 'devolucao' then
          begin
            if frmmodulo.qrproduto.Locate('codigo',
              qrnota_item.fieldbyname('codproduto').asstring,
              [loCaseInsensitive]) then
            begin
              if qrNFE_Cliente.fieldbyname('uf').asstring <>
                query2.fieldbyname('UF').asstring then
                Prod.cfop := frmmodulo.qrproduto.fieldbyname
                  ('IND_CFOP_DEVOLUCAO_FORA').asstring
              else
                Prod.cfop := frmmodulo.qrproduto.fieldbyname
                  ('IND_CFOP_DEVOLUCAO_DENTRO').asstring;
            end;
            Ide.natOp := frmmodulo.qrconfig.fieldbyname
              ('TEXTO_PADRAO_DEVOLUCAO').asstring;
          end;

          if qryapoio.fieldbyname('natureza').asstring = 'garantia' then
          begin
            if frmmodulo.qrproduto.Locate('codigo',
              qrnota_item.fieldbyname('codproduto').asstring,
              [loCaseInsensitive]) then
            begin
              if qrNFE_Cliente.fieldbyname('uf').asstring <>
                query2.fieldbyname('UF').asstring then
                Prod.cfop := frmmodulo.qrproduto.fieldbyname
                  ('IND_CFOP_GARANTIA_FORA').asstring
              else
                Prod.cfop := frmmodulo.qrproduto.fieldbyname
                  ('IND_CFOP_GARANTIA_DENTRO').asstring;
            end;
            Ide.natOp := frmmodulo.qrconfig.fieldbyname
              ('TEXTO_PADRAO_GARANTIA').asstring;
          end;
      ...
      Transp.Transporta.xNome := Trim(qrtransportador2.fieldbyname('nome')
        .asstring);
      Transp.Transporta.IE := qrtransportador2.fieldbyname('rg').asstring;
      Transp.Transporta.xEnder := Trim(qrtransportador2.fieldbyname('endereco')
        .asstring) + ', ' + Trim(qrtransportador2.fieldbyname('numero')
        .asstring);
...
begin
   AcbrnfeDanfefr1.FastFile := 'C:\Athenas_Retaguarda\SERVER\rel\DANFE.fr3';
  if sUsa_NFE = '1' then
  begin
    pn_nfe.Visible := True;
    Inicia_NFe();
    ...
    begin
      ...
      begin
        ...
        if not Exp_Inclui_item(qrnota_item.fieldbyname('item').asinteger,
          ...
          qrnota_item.fieldbyname('ICMS_REDUZIDO').asfloat,
          // ICMS_Reducao:REAL;
  ...
  case ACBrNFe1.status of
    ...
    stNFeStatusServico:
      begin
        ...
        frmStatus.lblStatus.Caption :=
          'Verificando Status do Servico, Aguarde...';
        frmStatus.Show;
    ...
    stNFeRecepcao:
      begin
        if (frmStatus = nil) then
          frmStatus := TfrmStatus.Create(Application);
        frmStatus.lblStatus.Caption := 'Enviando dados da NFe, Aguarde...';
        frmStatus.Show;
        frmStatus.BringToFront;
      end;
    stNfeRetRecepcao:
      begin
        if (frmStatus = nil) then
          frmStatus := TfrmStatus.Create(Application);
        frmStatus.lblStatus.Caption := 'Recebendo Dados da NFe, Aguarde...';
        frmStatus.Show;
        frmStatus.BringToFront;
      end;
    stNfeConsulta:
      begin
        if (frmStatus = nil) then
          frmStatus := TfrmStatus.Create(Application);
        frmStatus.lblStatus.Caption := 'Consultando NFe, Aguarde...';
        frmStatus.Show;
        frmStatus.BringToFront;
      end;
    stNfeCancelamento:
      begin
        if (frmStatus = nil) then
          frmStatus := TfrmStatus.Create(Application);
        frmStatus.lblStatus.Caption :=
          'Enviando Cancelamento de NFe, Aguarde...';
        frmStatus.Show;
        frmStatus.BringToFront;
      end;
    stNfeInutilizacao:
      begin
        if (frmStatus = nil) then
          frmStatus := TfrmStatus.Create(Application);
        frmStatus.lblStatus.Caption :=
          'Enviando Pedido de Inutilização, Aguarde...';
        frmStatus.Show;
        frmStatus.BringToFront;
      end;
    stNFeRecibo:
      begin
        if (frmStatus = nil) then
          frmStatus := TfrmStatus.Create(Application);
        frmStatus.lblStatus.Caption := 'Consultando Recibo de Lote, Aguarde...';
        frmStatus.Show;
        frmStatus.BringToFront;
      end;
    stNFeCadastro:
      begin
        if (frmStatus = nil) then
          frmStatus := TfrmStatus.Create(Application);
        frmStatus.lblStatus.Caption := 'Consultando Cadastro, Aguarde...';
        frmStatus.Show;
        frmStatus.BringToFront;
      end;
    stNFeEnvDPEC:
      begin
        if (frmStatus = nil) then
          frmStatus := TfrmStatus.Create(Application);
        frmStatus.lblStatus.Caption := 'Enviando DPEC, Aguarde...';
        frmStatus.Show;
        frmStatus.BringToFront;
      end;
    stNFeConsultaDPEC:
      begin
        if (frmStatus = nil) then
          frmStatus := TfrmStatus.Create(Application);
        frmStatus.lblStatus.Caption := 'Consultando DPEC, Aguarde...';
        frmStatus.Show;
        frmStatus.BringToFront;
      end;
    stNFeEmail:
...
//    ACBrNFeDANFERave1.ProtocoloNFe := '';
...
begin
  try
    ...
    begin
     ...
     pdf:= 'C:\Z5\SERVER\PDF\'+ stringreplace( ExtractFileName(sXML), '-NFe.xml' , '.pdf',[rfReplaceAll, rfIgnoreCase]);
     ...
     if FileExists(pdf) then sXML := sXML + '|'+ pdf;
      ...
      if email.EnviarEmail(edtnfe.Text, Para, edtEmailAssunto.Text,
...
begin
 ...
 if frmprincipal.autentica('Cancelar NF',4) then
  ...
  if sxml = '' then exit;
    ...
    begin
	  ...
	  idLote := '1';
	  if not(InputQuery('NFe Cancelamento por Evento', 'Identificador de controle do Lote de envio do Evento', idLote)) then
       exit;
      if not(InputQuery('NFe Cancelamento por Evento', 'Informe a justificativa', vAux)) then
         exit;
 ...
 //   WBResposta.Lines.Text :=  UTF8Encode(ACBrNFe1.WebServices.EnvEvento.RetornoWS);
