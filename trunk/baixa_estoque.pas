...
uses
  ...
  RXCtrls, Menus, RpDefine, RpCon, RpConDS, Wwdotdot, Wwdbcomb,
  AdvGlowButton, AdvShapeButton, frxDesgn, frxClass, frxDBSet, RxCurrEdit,
  RxToolEdit, UCBase;
...
begin
  ...
  begin
    ...
    begin
      NomeDoLog := 'C:\Z5\SERVER\Logs\TRANSFERENCIA_PRODUTOS' +
        DBEdit1.Text + '.txt';
...
begin
  ...
  if wwdbComboBox1.ITEMINDEX = 4 then
  begin
    fxorcamento.LoadFromFile('\Z5\server\rel\f000069_FILIAL.fr3');
    fxorcamento.ShowReport;
  end
  else
  begin
    fxorcamento.LoadFromFile('\Z5\server\rel\f000069_2.fr3');
    fxorcamento.ShowReport;
  end;
...
begin
  ...
  begin
    ...
    if frmmodulo.qrproduto.Locate('codigo', eproduto.Text, [loCaseInsensitive])
    then
    begin
      qritem_baixa_produtos.fieldbyname('produto').asstring :=
