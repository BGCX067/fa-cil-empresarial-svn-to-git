...
uses
  ...
  TFlatPanelUnit, Spin, Mask, RXDBCtrl, DBCtrls, DB,
  ...
  RzCmboBx, RzDBEdit, RzDBBnEd, RzBorder, RxToolEdit, RxCurrEdit;
...
type
  Tfrmconfig = class(TForm)
    ...
    DBEdit66: TDBEdit;
    DBEdit67: TDBEdit;
    btARRUMACODPRODUTO: TAdvGlowButton;
    qrestoque: TZQuery;
    ...
    procedure btARRUMACODPRODUTOClick(Sender: TObject);
...
{  if DBComboBox3.text = 'DARUMA' then
  begin
     executeFile('\Z5\server\ecf\daruma\Exemplo\Delphi\Daruma_Framework_Delphi7.exe', '', '',1)
  end; }
end;
...
procedure Tfrmconfig.btARRUMACODPRODUTOClick(Sender: TObject);
begin
  qrestoque.close;
  qrestoque.SQL.Clear;
  qrestoque.SQL.Add('select * from C000100');
  qrestoque.Open;
  qrestoque.First;
  while not qrestoque.Eof do
  begin
    qrestoque.Edit;
    qrestoque.FieldByName('CODPRODUTO').AsString := frmPrincipal.zerarcodigo(qrestoque.FieldByName('CODPRODUTO').AsString, 6);
    qrestoque.Post;
    qrestoque.Next;
  end;
