...
uses
  ...
  ZDataset, AdvGlowButton, AdvShapeButton, AdvOfficeHint, RxToolEdit, RxCurrEdit;
...
var texto: PWideChar;
  vstatus: string;
begin
   //and (rconvenio.Value = 0.00) and (rfinanceira.Value = 0.00)
  if (rdinheiro.Value = 0.00) and (rchequeav.Value = 0.00) and
    (rchequeap.Value = 0.00) and (rcartaocred.Value = 0.00) and (rcartaodeb.Value = 0.00)
    and (rcrediario.Value = 0.00)
    and (rsoma.Value = 0.00) and (rdiferenca.Value = 0.00) then
  begin
    Exit;
  end;
...
{  18 :qrcaixa_mov.fieldbyname('descricao_movimento').asstring := 'O.S. DINHEIRO';
  ...
  ////  LANCAMENTOS ESPECIFICOS DE CADA FORMA DE PAGAMENTO
      ...
      begin
        ...
        copy( 'O.S. No. ' + FRMMODULO.cdsos.FIELDBYNAME('CODIGO').ASSTRING
        + ' - ' + FRMMODULO.cdsos.FIELDBYNAME('CLIENTE').ASSTRING,1,60) ;
