object frmDoc_papel_cte: TfrmDoc_papel_cte
  Left = 613
  Top = 249
  BorderIcons = []
  BorderStyle = bsSingle
  Caption = 'Documento em Papel'
  ClientHeight = 179
  ClientWidth = 589
  Color = clBtnFace
  Font.Charset = DEFAULT_CHARSET
  Font.Color = clWindowText
  Font.Height = -11
  Font.Name = 'MS Sans Serif'
  Font.Style = []
  OldCreateOrder = False
  PopupMenu = pop1
  Position = poMainFormCenter
  PixelsPerInch = 96
  TextHeight = 13
  object RzPanel2: TRzPanel
    Left = 0
    Top = 118
    Width = 589
    Height = 61
    Align = alBottom
    BorderOuter = fsNone
    BorderColor = clBlack
    BorderWidth = 1
    Color = clWhite
    TabOrder = 0
    object btincluir: TAdvGlowButton
      Left = 185
      Top = 3
      Width = 93
      Height = 55
      Caption = 'F2 | Salvar'
      Font.Charset = ANSI_CHARSET
      Font.Color = clWindowText
      Font.Height = -11
      Font.Name = 'Tahoma'
      Font.Style = []
      ImageIndex = 49
      FocusType = ftHot
      NotesFont.Charset = DEFAULT_CHARSET
      NotesFont.Color = clWindowText
      NotesFont.Height = -11
      NotesFont.Name = 'Tahoma'
      NotesFont.Style = []
      ParentFont = False
      Picture.Data = {
        
        82}
      Transparent = True
      TabOrder = 1
      OnClick = btincluirClick
      Appearance.BorderColor = 14727579
      Appearance.BorderColorHot = 10079963
      Appearance.BorderColorDown = 4548219
      Appearance.BorderColorChecked = clBlack
      Appearance.Color = 15653832
      Appearance.ColorTo = 16178633
      Appearance.ColorChecked = 7915518
      Appearance.ColorCheckedTo = 11918331
      Appearance.ColorDisabled = 15921906
      Appearance.ColorDisabledTo = 15921906
      Appearance.ColorDown = 7778289
      Appearance.ColorDownTo = 4296947
      Appearance.ColorHot = 15465983
      Appearance.ColorHotTo = 11332863
      Appearance.ColorMirror = 15586496
      Appearance.ColorMirrorTo = 16245200
      Appearance.ColorMirrorHot = 5888767
      Appearance.ColorMirrorHotTo = 10807807
      Appearance.ColorMirrorDown = 946929
      Appearance.ColorMirrorDownTo = 5021693
      Appearance.ColorMirrorChecked = 10480637
      Appearance.ColorMirrorCheckedTo = 5682430
      Appearance.ColorMirrorDisabled = 11974326
      Appearance.ColorMirrorDisabledTo = 15921906
      Appearance.GradientHot = ggVertical
      Appearance.GradientMirrorHot = ggVertical
      Appearance.GradientDown = ggVertical
      Appearance.GradientMirrorDown = ggVertical
      Appearance.GradientChecked = ggVertical
    end
    object AdvGlowButton14: TAdvGlowButton
      Left = 278
      Top = 3
      Width = 107
      Height = 55
      Caption = 'ESC | Sair'
      Font.Charset = ANSI_CHARSET
      Font.Color = clWindowText
      Font.Height = -11
      Font.Name = 'Tahoma'
      Font.Style = []
      ImageIndex = 49
      FocusType = ftHot
      NotesFont.Charset = DEFAULT_CHARSET
      NotesFont.Color = clWindowText
      NotesFont.Height = -11
      NotesFont.Name = 'Tahoma'
      NotesFont.Style = []
      ParentFont = False
      Picture.Data = {
        
        A13EB9F389EDB60000000049454E44AE426082}
      Transparent = True
      TabOrder = 0
      OnClick = AdvGlowButton14Click
      Appearance.BorderColor = 14727579
      Appearance.BorderColorHot = 10079963
      Appearance.BorderColorDown = 4548219
      Appearance.BorderColorChecked = clBlack
      Appearance.Color = 15653832
      Appearance.ColorTo = 16178633
      Appearance.ColorChecked = 7915518
      Appearance.ColorCheckedTo = 11918331
      Appearance.ColorDisabled = 15921906
      Appearance.ColorDisabledTo = 15921906
      Appearance.ColorDown = 7778289
      Appearance.ColorDownTo = 4296947
      Appearance.ColorHot = 15465983
      Appearance.ColorHotTo = 11332863
      Appearance.ColorMirror = 15586496
      Appearance.ColorMirrorTo = 16245200
      Appearance.ColorMirrorHot = 5888767
      Appearance.ColorMirrorHotTo = 10807807
      Appearance.ColorMirrorDown = 946929
      Appearance.ColorMirrorDownTo = 5021693
      Appearance.ColorMirrorChecked = 10480637
      Appearance.ColorMirrorCheckedTo = 5682430
      Appearance.ColorMirrorDisabled = 11974326
      Appearance.ColorMirrorDisabledTo = 15921906
      Appearance.GradientHot = ggVertical
      Appearance.GradientMirrorHot = ggVertical
      Appearance.GradientDown = ggVertical
      Appearance.GradientMirrorDown = ggVertical
      Appearance.GradientChecked = ggVertical
    end
  end
  object sGroupBox1: TsGroupBox
    Left = 0
    Top = 0
    Width = 589
    Height = 118
    Align = alClient
    TabOrder = 1
    SkinData.SkinSection = 'GROUPBOX'
    object Label1: TLabel
      Left = 282
      Top = 17
      Width = 24
      Height = 13
      Caption = 'S'#233'rie'
      FocusControl = serie
    end
    object Label2: TLabel
      Left = 355
      Top = 17
      Width = 41
      Height = 13
      Caption = 'Subserie'
      FocusControl = subserie
    end
    object Label3: TLabel
      Left = 24
      Top = 64
      Width = 90
      Height = 13
      Caption = 'N'#250'mero Doc Fiscal'
      FocusControl = numerodoc
    end
    object Label4: TLabel
      Left = 155
      Top = 64
      Width = 65
      Height = 13
      Caption = 'Data Emiss'#227'o'
      FocusControl = data
    end
    object Label5: TLabel
      Left = 28
      Top = 17
      Width = 141
      Height = 13
      Caption = 'Tipo do Documento Origin'#225'rio'
      FocusControl = serie
    end
    object serie: TcxDBTextEdit
      Left = 282
      Top = 33
      DataBinding.DataField = 'SERIE'
      DataBinding.DataSource = DataSource1
      TabOrder = 1
      Width = 58
    end
    object subserie: TcxDBTextEdit
      Left = 355
      Top = 33
      DataBinding.DataField = 'SUB_SERIE'
      DataBinding.DataSource = DataSource1
      TabOrder = 2
      Width = 62
    end
    object numerodoc: TcxDBTextEdit
      Left = 24
      Top = 80
      DataBinding.DataField = 'NUMERO_DOC'
      DataBinding.DataSource = DataSource1
      TabOrder = 3
      Width = 121
    end
    object data: TcxDBDateEdit
      Left = 155
      Top = 80
      DataBinding.DataField = 'DATA_EMISSAO'
      DataBinding.DataSource = DataSource1
      TabOrder = 4
      Width = 121
    end
    object tipodoc: TDBComboBox
      Left = 24
      Top = 34
      Width = 254
      Height = 21
      DataField = 'TIPO_DOC'
      DataSource = DataSource1
      Items.Strings = (
        'CTRC'
        'CTAC'
        'ACT'
        'NF Modelo 7'
        'NF Modelo 27'
        'Conhecimento A'#233'reo Nacional'
        'CTMC'
        'ATRE'
        'DTA (despacho de transito aduaneiro)'
        'Conhecimento A'#233'reo Internacional'
        'Conhecimento - Carta de Porte Internacional'
        'Conhecimento Avulso'
        'TIF (transporte internacional ferrovi'#225'rio)'
        'Outros')
      TabOrder = 0
    end
  end
  object DataSource1: TDataSource
    DataSet = frmmodulo.qrdoc_ant_papel
    Left = 288
    Top = 152
  end
  object ACBrEnterTab1: TACBrEnterTab
    EnterAsTab = True
    Left = 464
    Top = 24
  end
  object pop1: TPopupMenu
    Left = 464
    Top = 56
  end
end
