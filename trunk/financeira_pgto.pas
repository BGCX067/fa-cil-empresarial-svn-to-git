...
begin
  ...
  frmprincipal.logUC('Baixou Mov.Financeira - CodVenda: ' +
    frmfinanceira.qrfinanceira.fieldbyname('CODVENDA').asstring, 3);

  frmfinanceira.query.close;
  frmfinanceira.query.sql.clear;
  frmfinanceira.query.sql.add
    ('update c000123 set situacao = 2, valor_pago = :valor_pago, data_pgto = :data_pgto');
  frmfinanceira.query.sql.add('where codigo = ''' +
    frmfinanceira.qrfinanceira.fieldbyname('codigo').asstring + '''');
