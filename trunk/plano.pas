...
uses
  ...
  Wwdbcomb, PageView, ImgList, ComCtrls, AdvGlowButton, UCBase, W7Labels, dxGDIPlusClasses, RzPanel;
  ...
  begin
    ...
    begin
      Application.MESSAGEBOX
        ('Conta de Nível Superior do Sistema! Impossível excluir!', 'Atenção',
        mb_ok + MB_ICONerror);
      exit;
    end;
    ...
    begin
      if Application.MESSAGEBOX
        ('Esta conta é um subgrupo. Caso você exclua esta conta, todas as contas dentro dela serão excluídas! Deseja prosseguir?',
        'Atenção', mb_yesno + MB_ICONWARNING + MB_DEFBUTTON2) = idyes then
      begin
        query1.close;
        query1.SQL.clear;
        query1.SQL.add('delete from c000035 where codgrupo =''' +
          frmmodulo.qrplano.fieldbyname('codigo').asstring + '''');
        query1.ExecSQL;
...
begin
  ...
  begin
    frmmodulo.qrplano.Locate('codigo', resultado_pesquisa1,
      [loCaseInsensitive]);
  end;
end;
...
begin
  ...
  begin
    Application.MESSAGEBOX('Favor informar o nome da conta!', 'Atenção',
      mb_ok + MB_ICONerror);
    DBEdit2.setfocus;
    exit;
  end;
...
begin
  if (frmmodulo.qrplano.State = dsinsert) or (frmmodulo.qrplano.State = dsedit)
  then
  begin
    if (frmmodulo.qrplano.State = dsinsert) then
    begin
      ...
      frmprincipal.qrmestre.SQL.add
        ('update c000000 set sequencia = sequencia - 1 where codigo = ''000035''');
...
begin
  ...
  begin
    ...
    begin
      ...
      begin
        frmmodulo.qrplano.fieldbyname('codgrupo').asstring :=
          resultado_pesquisa1;
        query1.close;
        query1.SQL.clear;
        query1.SQL.add('select * from c000035 where classificacao like ''' +
          resultado_pesquisa3 + '%'' and nivel = 3 order by classificacao');
        query1.open;
        if query1.RecordCount > 0 then
        begin
          query1.last;
          conta := copy(query1.fieldbyname('classificacao').asstring, 6, 3);
          conta := resultado_pesquisa3 + '.' + frmprincipal.texto_justifica
            (inttostr(strtoint(conta) + 1), 3, '0', 'E');
        end
        else
        begin
      ...
      begin
        Application.MESSAGEBOX('Conta inválida para subgrupo!', 'Atenção',
    ...
    begin
      ...
      if COMBOGRUPO.ItemIndex = 0 then
        query1.SQL.add
          ('select * from c000035 where classificacao like ''2%'' and nivel = 2 order by classificacao');
      if COMBOGRUPO.ItemIndex = 1 then
        query1.SQL.add
          ('select * from c000035 where classificacao like ''1%'' and nivel = 2 order by classificacao');
      query1.open;
      if query1.RecordCount > 0 then
      begin
        query1.last;
        ...
        if COMBOGRUPO.ItemIndex = 0 then
          conta := '2.' + frmprincipal.texto_justifica
            (inttostr(strtoint(conta) + 1), 2, '0', 'E');
        if COMBOGRUPO.ItemIndex = 1 then
          conta := '1.' + frmprincipal.texto_justifica
            (inttostr(strtoint(conta) + 1), 2, '0', 'E');
      end
      else
      begin
...
begin
  ...
  begin
    ...
    begin
      ...
      begin
        with arvore.Items.AddChild(nil, query1.fieldbyname('classificacao')
          .asstring + ' ' + query1.fieldbyname('conta').asstring) do
        begin
      ...
      begin
        with arvore.Items.AddChild(topo, query1.fieldbyname('classificacao')
          .asstring + ' ' + query1.fieldbyname('conta').asstring) do
        begin
      ...
      begin
        with arvore.Items.AddChild(arvore.Selected,
          query1.fieldbyname('classificacao').asstring + ' ' +
          query1.fieldbyname('conta').asstring) do
        begin
          if query1.fieldbyname('tipo').ASINTEGER = 1 then
          begin
            SelectedIndex := 3;
