...
uses
  ...
  ZConnection, Mask, rxToolEdit, ExtCtrls, RzPanel, MemDS,
  ...
  RxCurrEdit, ACBrBase, ACBrValidador, AdvReflectionImage, wwdbedit;
...
type
  Tfrmimporta_nfe = class(TForm)
    ...
    qritemLOTE_FABRICACAO_DATA: TDateTimeField;
    ...
    QryCadastraProdutoDATA_CADASTRO: TDateTimeField;
    ...
    QryCadastraProdutoDATA_ULTIMACOMPRA: TDateTimeField;
    ...
    QryCadastraProdutoDATA_ULTIMAVENDA: TDateTimeField;
    ...
    QryCadastraProdutoDATA_ULTIMACOMPRA_ANTERIOR: TDateTimeField;
    ...
    QryCadastraProdutoDATA_REMARCACAO_CUSTO: TDateTimeField;
    QryCadastraProdutoDATA_REMARCACAO_VENDA: TDateTimeField;
    QryCadastraProdutoPRECO_PROMOCAO: TFloatField;
    QryCadastraProdutoDATA_PROMOCAO: TDateTimeField;
    QryCadastraProdutoFIM_PROMOCAO: TDateTimeField;
    ...
    QryCadastraProdutoULTIMA_ALTERACAO: TDateTimeField;
    QryCadastraProdutoULTIMA_CARGA: TDateTimeField;
    QryCadastraProdutoDATA_INVENTARIO: TDateTimeField;
    ...
    QryCadastraProdutoDATA_INVENTARIO_ATUAL: TDateTimeField;
    ...
    QryCadastraProdutoLOTE_VALIDADE: TDateTimeField;
    ...
    QryCadastraProdutoULTIMA_COMPRA: TDateTimeField;
    QryCadastraProdutoFARMACIA_DATAVIGENCIA: TDateTimeField;
    ...
    qryestoqueULTIMA_ENTRADA: TDateTimeField;
    qryestoqueULTIMA_SAIDA: TDateTimeField;
    ...
    bmargem: TAdvGlowButton;
    qrproduto_movCONTROLE: TWideStringField;
    ...
    procedure bmargemClick(Sender: TObject);
  private
...
uses ACBrNFeNotasFiscais, pcnNFe, MaskUtils, loc_conta, modulo, principal,
  Produto, produto_ligth, produto_farma, produto_auto, UFuncoes, uBuscaXml, Ncm, cst, MargemLucro;
...
Application.ProcessMessages;
 ...
 try
   ...
   qritemPrecoVenda.Value := ((qritemPRECOCUSTO.AsFloat) * qritemPMARGEM.AsFloat / 100) + (qritemPRECOCUSTO.AsFloat);
 except
...
try
   { calcula o preco de venda}
   qritemPMARGEM.Value := (((qritemPrecoVenda.AsFloat - qritemPRECOCUSTO.AsFloat) / qritemPRECOCUSTO.AsFloat))  * 100;
 except
    ...
    inicia: { : INICIA VERIFICAÇÃO SE O PRODUTO ESTÁ PRONTO PARA RECEBER MOVIMENTAÇÃO - HISTORICO DE COMPRAS }
      ...
      begin
        ...
        qrproduto_movCODNOTA.AsString := qritem.fieldbyname('codnota')
          .AsString + 'NE';
        ///modificado aqui para exclusao da nota de entrada // Valtenci 25.02.2014
        qrproduto_movCONTROLE.AsString := qritem.fieldbyname('codnota').AsString;
...
try
   ...
   {pega os valores originais}
    qritemPRECOCUSTO.AsFloat := (qritemTOTAL.asfloat + qritemFRETE.asfloat + qritemSEGURO.asfloat + qritemOUTRAS.asfloat + qritemIPI_VALOR.asfloat) / (qritemQTDE.asfloat) / qritemFRACAO.asfloat;

    if qritemPMARGEM.AsFloat > 0 then
      qritemPrecoVenda.AsFloat := (qritemPRECOCUSTO.AsFloat * qritemPMARGEM.AsFloat) / 100 + qritemPRECOCUSTO.AsFloat
    else
      qritemPrecoVenda.AsFloat := qritemPRECOCUSTO.AsFloat;
...
procedure Tfrmimporta_nfe.bmargemClick(Sender: TObject);
begin
///margem de lucro
 ...
 // frmPrincipal.Aguarde('Aguarde','Aplicando margem de lucro...',false,true,0);
  while not qritem.Eof do
  begin
    qritem.Edit;
    qritemPMARGEM.Value := pMargem;
    qritemPrecoVenda.Value := ((qritemPRECOCUSTO.AsFloat) * pMargem / 100) + (qritemPRECOCUSTO.AsFloat);
    qritem.Post;
    qritem.Next;
    Application.ProcessMessages;
  end;
...
begin
  ...
  fximprime.LoadFromFile('\Athenas\server\REL\F000197.fr3');
