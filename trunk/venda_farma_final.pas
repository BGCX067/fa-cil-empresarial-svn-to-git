...
uses
  ...
  ZDataset, Grids, Wwdbigrd, Wwdbgrid, Mask, wwdbedit,
  Wwdotdot, Wwdbcomb, AdvGlowButton, RxToolEdit, RxCurrEdit;
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        begin
          texto := PWideChar('Cliente com Prestação vencida a ' + FloatToStr(dias) + ' Dias!, Deseja continuar?');
    ...
    begin
      ...
      begin
        ...
        begin
          texto := PWideChar('Este cliente possue um limite disponível de:' + #13 + ' ---> R$ ' + lcliente_limite.caption + '  <--- ' + #13 + 'É necessário autorização para prosseguir! ' + #13 + 'Deseja continuar?');
          if application.MessageBox(texto, 'Atenção', mb_yesno + MB_ICONWARNING) = idyes then
          begin
            if not frmPrincipal.autentica('Liberar venda', 4) then
            begin
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        texto := PWideChar('Este cliente possue convênio: ' + #13 + query1.fieldbyname('codigo').asstring + ' - ' + query1.fieldbyname('nome').asstring + #13 + 'Deseja reverter este fechamento para o CONVÊNIO?');
        if Application.MessageBox(TEXTO, 'Atenção', mb_yesno + mb_iconwarning) = idyes then
        begin
