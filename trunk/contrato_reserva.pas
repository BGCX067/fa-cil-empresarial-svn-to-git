...
begin
  ...
  // pessoa fisica
    fxcontrato.LoadFromFile('\Z5\server\rel\contrato_reserva.fr3')
  else
  begin
    // pessoa juridica
    if qrcliente.FieldByName('resp2_nome').asstring <> '' then
    // dois responsaveis
      fxcontrato.LoadFromFile('\Z5\server\rel\contrato_reserva_j2.fr3')
    else
    // um responsavel
      fxcontrato.LoadFromFile('\Z5\server\rel\contrato_reserva_j1.fr3')
...
begin
  arquivo_ini := TIniFile.Create('c:\Z5\server\dll\contrato.ini');
