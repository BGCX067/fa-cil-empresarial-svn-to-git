...
uses
  ...
  AdvGlowButton, RzPrgres, MemDS, DBAccess, IBC, RzEdit, RxMemDS, CurrEdit;
...
type
  Tfrmlista_produto2 = class(TForm)
    ...
    qrprodutoESTOQUE_FISCAL: TFloatField;
    ...
    qrprodutoMARKUP: TFloatField;
    qrprodutoCODPRODUTO: TStringField;
    qrprodutoCODFILIAL: TStringField;
    qrprodutoESTOQUE_FISCAL_1: TFloatField;
    Label13: TLabel;
    Label14: TLabel;
    rxqte1: TRxCalcEdit;
    rxqte2: TRxCalcEdit;
    qrprodutoCFOP: TStringField;
    qrprodutoEST_DEP: TFloatField;
    qrprodutoEST_EXP: TFloatField;
    qrprodutoEST_CON: TFloatField;
    ...
    procedure combo_relatorioExit(Sender: TObject);
    procedure rxqte1KeyPress(Sender: TObject; var Key: Char);
    procedure rxqte2KeyPress(Sender: TObject; var Key: Char);
  private
...
var
  situacao, PRODUTO,GRUPO, SUBGRUPO, FORNECEDOR, MARCA, ESTOQUE, ESTOQUE1, ESTOQUE2, INVENTARIO, CST, tipo,  ordem: string;
  v01,v07,v12,v17,v25,v27 : double;
  ESTOQUE_DEP, ESTOQUE_EXP, ESTOQUE_CON : string;
begin
...
//  qrproduto.LinkedFields := '';
  ...
  if combo_grupo.Text = 'TODOS' then grupo := '' else begin  grupo := ' and codgrupo = '''+copy(combo_grupo.text,1,3)+''''; frmmodulo.qrrelatorio.fieldbyname('LINHA3').asstring := 'GRUPO: '+COMBO_grupo.TEXT; END;
  ...
  begin
    ESTOQUE := '';
    ESTOQUE1 := '';
    ESTOQUE2 := ' AND ESTQ.ESTOQUE_ATUAL >= '+FloatToStr(rxqte1.Value) +' AND';
    ESTOQUE2 := ESTOQUE2 + ' ESTQ.ESTOQUE_ATUAL <= ' + FloatToStr(rxqte2.Value);
  ...
  begin
    ...
    ESTOQUE2 := ' AND ESTQ.ESTOQUE_ATUAL >= '+FloatToStr(rxqte1.Value) +' AND';
    ESTOQUE2 := ESTOQUE2 + ' ESTQ.ESTOQUE_ATUAL <= ' + FloatToStr(rxqte2.Value);
    ESTOQUE_DEP := ' AND ESTQ.EST_DEP > 0 ';
    ESTOQUE_EXP := ' AND ESTQ.EST_EXP > 0 ';
    ESTOQUE_CON := ' AND ESTQ.EST_CON > 0 ';
  end;
  ...
  begin
    ...
    ESTOQUE_DEP := ' AND ESTQ.EST_DEP < 1 ';
    ESTOQUE_EXP := ' AND ESTQ.EST_EXP < 1 ';
    ESTOQUE_CON := ' AND ESTQ.EST_CON < 1 ';
...
//    qrposicao.SQL.add('or (c000100.estoque_inicial > 0 )');
    ...
    fXproduto.LoadFromFile('\siscom\server\rel\f000184.fr3');
  ...
  begin
//    textosql := 'select prod.*, estq.* from c000025 prod, c000100 estq where prod.codigo = estq.codproduto ';
//    textosql :=  textosql + estoque+ 'AND CODGRUPO IS NULL'+SUBGRUPO+fornecedor+marca+tipo+situacao+cst+' order by '+ordem;
 //   showmessage(textosql);
    qrPRODUTO.close;
    qrPRODUTO.sql.clear;
...
//    qrproduto.sql.add(estoque+grupo+SUBGRUPO+fornecedor+marca+tipo+situacao+cst+' order by '+ordem);
    qrproduto.sql.add(estoque+ 'AND CODGRUPO IS NULL'+SUBGRUPO+fornecedor+marca+tipo+situacao+cst+' order by '+ordem);
    qrPRODUTO.open;
    frmmodulo.qrrelatorio.fieldbyname('LINHA1').asstring := 'RELAÇÃO DE PRODUTOS - SINTÉTICA';
    fXproduto.LoadFromFile('\siscom\server\rel\f000025.fr3');
    fxproduto.ShowReport;
  end;

 if combo_relatorio.Text = 'RELAÇÃO GERAL DE ESTOQUE - SINTÉTICA' then
  begin
//    textosql := 'select prod.*, estq.* from c000025 prod, c000100 estq where prod.codigo = estq.codproduto ';
//    textosql :=  textosql + estoque+ 'AND CODGRUPO IS NULL'+SUBGRUPO+fornecedor+marca+tipo+situacao+cst+' order by '+ordem;
 //   showmessage(textosql);
...
//    qrproduto.sql.add(estoque+grupo+SUBGRUPO+fornecedor+marca+tipo+situacao+cst+' order by '+ordem);
    qrproduto.sql.add(estoque+ 'AND CODGRUPO IS NULL'+SUBGRUPO+fornecedor+marca+tipo+situacao+cst+' order by '+ordem);
    qrPRODUTO.open;
    frmmodulo.qrrelatorio.fieldbyname('LINHA1').asstring := 'RELAÇÃO DE PRODUTOS GERAL - SINTÉTICA';
    fXproduto.LoadFromFile('\siscom\server\rel\f000025g.fr3');
    fxproduto.ShowReport;
  end;


  if combo_relatorio.Text = 'RELAÇÃO DE ESTOQUE SINTÉTICA - DEPÓSITO' then
  begin
//    textosql := 'select prod.*, estq.* from c000025 prod, c000100 estq where prod.codigo = estq.codproduto ';
//    textosql :=  textosql + estoque+grupo+SUBGRUPO+fornecedor+marca+tipo+situacao+cst+' order by '+ordem;
//    showmessage(textosql);
    qrPRODUTO.close;
    qrPRODUTO.sql.clear;
    qrPRODUTO.sql.add('select prod.*, estq.* from c000025 prod, c000100 estq where prod.codigo = estq.codproduto ');
    qrproduto.sql.add(estoque_dep+grupo+SUBGRUPO+fornecedor+marca+tipo+situacao+cst+' order by '+ordem);
    qrPRODUTO.open;
    frmmodulo.qrrelatorio.fieldbyname('LINHA1').asstring := 'RELAÇÃO DE PRODUTOS - DEPÓSITO';
    fXproduto.LoadFromFile('\siscom\server\rel\f000025d.fr3');
    fxproduto.ShowReport;
  end;
  if combo_relatorio.Text = 'RELAÇÃO DE ESTOQUE SINTÉTICA - EXPOSITOR' then
  begin
//    textosql := 'select prod.*, estq.* from c000025 prod, c000100 estq where prod.codigo = estq.codproduto ';
//    textosql :=  textosql + estoque+grupo+SUBGRUPO+fornecedor+marca+tipo+situacao+cst+' order by '+ordem;
//    showmessage(textosql);
    ...
    qrPRODUTO.sql.clear;
    qrPRODUTO.sql.add('select prod.*, estq.* from c000025 prod, c000100 estq where prod.codigo = estq.codproduto ');
    qrproduto.sql.add(estoque_exp+grupo+SUBGRUPO+fornecedor+marca+tipo+situacao+cst+' order by '+ordem);
    qrPRODUTO.open;
    frmmodulo.qrrelatorio.fieldbyname('LINHA1').asstring := 'RELAÇÃO DE PRODUTOS - EXPOSITOR';
    fXproduto.LoadFromFile('\siscom\server\rel\f000025e.fr3');
    fxproduto.ShowReport;
  end;
  if combo_relatorio.Text = 'RELAÇÃO DE ESTOQUE SINTÉTICA - CONVENIÊNCIA' then
  begin
//    textosql := 'select prod.*, estq.* from c000025 prod, c000100 estq where prod.codigo = estq.codproduto ';
//    textosql :=  textosql + estoque+grupo+SUBGRUPO+fornecedor+marca+tipo+situacao+cst+' order by '+ordem;
 //   showmessage(textosql);
    ...
    qrproduto.sql.add(estoque+grupo+SUBGRUPO+fornecedor+marca+tipo+situacao+cst+' order by '+ordem);
    qrPRODUTO.open;
    frmmodulo.qrrelatorio.fieldbyname('LINHA1').asstring := 'RELAÇÃO DE PRODUTOS - CONVENIENCIA';
    fXproduto.LoadFromFile('\siscom\server\rel\f000025c.fr3');
    fxproduto.ShowReport;
  end;
  ...
  begin
    ...
    fXproduto.LoadFromFile('\siscom\server\rel\f000076.fr3');
    fxproduto.ShowReport;
  end;
  ...
  begin
    ...
    fXproduto.LoadFromFile('\siscom\server\rel\f000026.fr3');
    fxproduto.ShowReport;
  end;
  ...
  begin
    ...
    fXproduto.LoadFromFile('\siscom\server\rel\f000075.fr3');
    fxproduto.ShowReport;
  end;
  ...
  begin
    ...
    fXPRODUTO.LoadFromFile('\siscom\server\rel\f000027.fr3');
    fxPRODUTO.ShowReport;
  end;
  ...
  begin
    ...
    fXproduto.LoadFromFile('\siscom\server\rel\f000028.fr3');
    fxproduto.ShowReport;
  end;
  ...
  begin
    ...
    fXproduto.LoadFromFile('\siscom\server\rel\f000029.fr3');
    fxproduto.ShowReport;
  end;
  ...
  begin
    ...
    fXproduto.LoadFromFile('\siscom\server\rel\f000030.fr3');
    fxproduto.ShowReport;
  end;
  ...
  begin
    ...
    fXproduto.LoadFromFile('\esiscom\server\rel\f000017.fr3');
    fxproduto.ShowReport;
  end;
  ...
  begin
    ...
    fXproduto.LoadFromFile('\siscom\server\rel\f000016.fr3');
    fxproduto.ShowReport;
  end;
  ...
  begin
    ...
    fXproduto.LoadFromFile('\siscom\server\rel\f000031.fr3');
    fxproduto.ShowReport;
  end;
  ...
  begin
    ...
    fXproduto.LoadFromFile('\siscom\server\rel\f000128.fr3');
    fxproduto.ShowReport;
  end;
  ...
  BEGIN
    ...
    fXproduto.LoadFromFile('\siscom\server\rel\f000020.fr3');
    fxproduto.ShowReport;
  END;
  ...
  BEGIN
    ...
    fXproduto.LoadFromFile('\siscom\server\rel\f000130.fr3');
    fxproduto.ShowReport;
  END;
...
//    qrproduto.LinkedFields := 'codsubgrupo';
    ...
    fXproduto.LoadFromFile('\siscom\server\rel\f000018.fr3');
    fxproduto.ShowReport;
    qrproduto.MasterSource := nil;
    qrproduto.MasterFields := '';
  ...
  begin
    ...
    fXproduto.LoadFromFile('\siscom\server\rel\f000101.fr3');
    fxproduto.ShowReport;
    qrproduto.MasterSource := nil;
    qrproduto.MasterFields := '';
    qrproduto.IndexFieldNames := '';
  ...
  begin
    ...
    fXproduto.LoadFromFile('\siscom\server\rel\f000102.fr3');
    fxproduto.ShowReport;
    qrproduto.MasterSource := nil;
    qrproduto.MasterFields := '';
    qrproduto.IndexFieldNames := '';
...
//    qrproduto.LinkedFields := 'codsubgrupo';
    ...
    fXproduto.LoadFromFile('\siscom\server\rel\f000019.fr3');
    fxproduto.ShowReport;
    qrproduto.MasterSource := nil;
    qrproduto.MasterFields := '';
    qrproduto.IndexFieldNames := '';
    ...
    query_pis_entrada.open;
{
    ...
    fXproduto.LoadFromFile('\siscom\server\rel\f000142_1.fr3');
    fxproduto.ShowReport;
  ...
  BEGIN
    ...
    fXproduto.LoadFromFile('\siscom\server\rel\f000142.fr3');
    fxproduto.ShowReport;
  ...
  BEGIN
    ...
    fXproduto.LoadFromFile('\siscom\server\rel\f000142_3.fr3');
    fxproduto.ShowReport;
  ...
  BEGIN
    ...
    fXproduto.LoadFromFile('\siscom\server\rel\f000142_2.fr3');
    fxproduto.ShowReport;
  ...
  begin
    ...
    fXproduto.LoadFromFile('\siscom\server\rel\f000098.fr3');
    fxproduto.ShowReport;
  end;
  ...
  begin
    ...
    fXproduto.LoadFromFile('\siscom\server\rel\f000103.fr3');
    fxproduto.ShowReport;
  end;
  ...
  BEGIN
    ...
    frmmodulo.qrrelatorio.fieldbyname('LINHA1').asstring := 'PRODUTOS - TABELA DE PREÇOS';
    FRMMODULO.QRRELATORIO.FieldByName('linha3').AsString := 'PERÍODO: '+ldataini_remarcado.text+' a '+ldatafim_remarcado.text;
    fXproduto.LoadFromFile('\siscom\server\rel\f000190.fr3');
    fxproduto.ShowReport;
  END;
  ...
  ///////////////////////////////////////////////////
      ...
      fXproduto.LoadFromFile('\siscom\server\rel\f000143.fr3');
      fxproduto.ShowReport;
  ...
  if combo_relatorio.Text = 'REGISTRO DE CONTROLE DA PRODUÇÃO E DO ESTOQUE' then
    begin
      ...
      fXproduto.LoadFromFile('\siscom\server\rel\f000196.fr3');
      fxproduto.ShowReport;
    end;

  if combo_relatorio.Text = 'RELAÇÃO DE ESTOQUE  FISCAL - SINTÉTICA' then
  begin
//    textosql := 'select prod.*, estq.* from c000025 prod, c000100 estq where prod.codigo = estq.codproduto ';
//    textosql :=  textosql + estoque+grupo+SUBGRUPO+fornecedor+marca+tipo+situacao+cst+' order by '+ordem;
//    showmessage(textosql);
    ...
    qrproduto.sql.add(estoque+grupo+SUBGRUPO+fornecedor+marca+tipo+situacao+cst+' order by '+ordem);
    qrPRODUTO.open;
    frmmodulo.qrrelatorio.fieldbyname('LINHA1').asstring := 'RELAÇÃO DE ESTOQUE  FISCAL - SINTÉTICA';
    fXproduto.LoadFromFile('\siscom\server\rel\f000252.fr3');
    fxproduto.ShowReport;
  end;

  if combo_relatorio.Text = 'RELAÇÃO DE PREÇOS COM MARK-UP' then
  begin
    qrPRODUTO.close;
    ...
    frmmodulo.qrrelatorio.fieldbyname('LINHA1').asstring := 'RELAÇÃO DE PREÇOS COM MARK-UP';
    fXproduto.LoadFromFile('\siscom\server\rel\f000031_1.fr3');
    fxproduto.ShowReport;

  end;
  if combo_relatorio.Text = 'RELAÇÃO DE ESTOQUE POR QUANTIDADE - SINTÉTICA' then
  begin
//    textosql := 'select prod.*, estq.* from c000025 prod, c000100 estq where prod.codigo = estq.codproduto ';
//    textosql :=  textosql + estoque+grupo+SUBGRUPO+fornecedor+marca+tipo+situacao+cst+' order by '+ordem;
//    showmessage(textosql);
    ...
    qrPRODUTO.sql.clear;
    qrPRODUTO.sql.add('select prod.*, estq.* from c000025 prod, c000100 estq where prod.codigo = estq.codproduto ');
    qrproduto.sql.add(estoque2+grupo+SUBGRUPO+fornecedor+marca+tipo+situacao+cst+' order by '+ordem);
    qrPRODUTO.open;
    frmmodulo.qrrelatorio.fieldbyname('LINHA1').asstring := 'RELAÇÃO DE PRODUTOS - SINTÉTICA - DE: ' + FloatToStr(rxqte1.value) +' ATE: '+ FloatToStr(rxqte2.value);
    fXproduto.LoadFromFile('\siscom\server\rel\f000253.fr3');
    fxproduto.ShowReport;
  end;
...
begin
  ...
  if combo_relatorio.Text = 'RELAÇÃO DE ESTOQUE - POR MARCA' then
  begin
    fXproduto.LoadFromFile('\siscom\server\rel\f000017.fr3');
    fxproduto.DESIGNREPORT;
  end;
...
procedure Tfrmlista_produto2.combo_relatorioExit(Sender: TObject);
begin
  if combo_relatorio.Text = 'RELAÇÃO DE ESTOQUE POR QUANTIDADE - SINTÉTICA' then
  begin
    rxqte1.Enabled := True;
...
procedure Tfrmlista_produto2.rxqte1KeyPress(Sender: TObject;
  var Key: Char);
begin
  if key = #13 then perform(wm_nextdlgctl,0,0);
...
procedure Tfrmlista_produto2.rxqte2KeyPress(Sender: TObject;
  var Key: Char);
begin
  if key = #13 then perform(wm_nextdlgctl,0,0);
