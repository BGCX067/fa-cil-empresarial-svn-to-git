...
uses
  ...
  RzCmboBx, RzDBEdit, RzDBBnEd, RzBorder, AdvReflectionImage;
...
type
  Tfrmconfig = class(TForm)
    ...
    DBCheckLMC: TDBCheckBox;
    rramo_posto: TRadioButton;
    ...
    procedure DBCheckLMCClick(Sender: TObject);
...
begin
 //  frmprincipal.AdvToolBarPager12.Refresh;
   impressora_venda := frmmodulo.qrconfig.fieldbyname('VENDA_PORTA_IMPRESSORA').asstring;
    ...
    if Rramo_posto.Checked then frmmodulo.qrconfig.FieldByName('ramo_atividade').asinteger := 5;
...
{  if DBComboBox3.text = 'DARUMA' then
  begin
     executeFile('\siscom\server\ecf\daruma\Exemplo\Delphi\Daruma_Framework_Delphi7.exe', '', '',1)
  end; }
end;
...
procedure Tfrmconfig.DBCheckLMCClick(Sender: TObject);
begin
frmprincipal.AdvToolBarPager12.Refresh; 
end;
