...
uses
  ...
  DBTables, ImgList, Menus, AdvGlowButton, ExtCtrls, RzPrgres,
      ...
      begin
        try
          ...
          textox := PWideChar('Não foi possível se conectar o banco de dados: ' + #13 +
            qrpdv.fieldbyname('hostname').asstring + ':c:\Z5\bd\base.fdb' + #13 +
...
begin
  ...
  AssignFile(TXT, 'c:\Z5\server\CG' + IntToStr(ANO) + IntToStr(MES) + IntToStr(DIA) + '_' + IntToStr(HORA) + IntToStr(MINUTO) + IntToStr(SEGUNDO) + '.001');
  ...
  bar.Percent := 0;
  origem := 'c:\Z5\server\CG' + IntToStr(ANO) + IntToStr(MES) + IntToStr(DIA) + '_' + IntToStr(HORA) + IntToStr(MINUTO) + IntToStr(SEGUNDO);
